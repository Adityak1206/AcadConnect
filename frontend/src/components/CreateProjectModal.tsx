...
export const CreateProjectModal = ({ isOpen, onClose, onProjectCreated }: CreateProjectModalProps) => {
  ...
  const fetchGroups = async () => {
    ...
    try {
      ...
      // Only show groups where the user has accepted status
      const eligibleGroups = allGroups.filter(g => g.my_status === 'accepted');
  ...
  const groupEligible = acceptedMemberCount >= 3 && acceptedMemberCount <= 5;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!groupEligible) {
      setError('Selected group must have 3–5 accepted members to create a project.');
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
          ...
          {/* Group Selection */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Select Your Group</label>
            {isLoadingGroups ? (
              ...
              <div className="bg-amber-50 text-amber-700 p-3 rounded-lg text-sm">
                You don't have any eligible groups. Create a group with 3–5 accepted members first.
              </div>
            ) : (
              <select
                required
                ...
                {groups.map((group) => {
                  ...
                  return (
                    <option key={group.group_id} value={group.group_id}>
                      {group.name} ({accepted} accepted members)
                    </option>
                  );
                })}
              </select>
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-100 p-4">
              ...
              {!groupEligible && (
                <div className="bg-red-50 text-red-600 p-2 rounded-lg text-xs font-medium mb-2">
                  This group needs 3–5 accepted members to create a project.
                </div>
              )}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={isLoading || !groupEligible || groups.length === 0}
